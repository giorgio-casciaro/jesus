...
var newConnection = function (path, debugData) {
  return new Promise((resolve, reject) => {
    ...
    zeromqClient.on('connect', async code => {
      ...
      log('zeromq  CLIENT message ', data)
      if ((!data.type || data.type === 'response') && data.reqId && zeromqClient.responsesInfo[data.reqId]) {
        ...
        log('zeromq  CLIENT message stream', data)
    ...
    zeromqClient.rpc = async (method, data, meta, asStream = false, timeout = 120000) => {
      ...
      if (!asStream) {
        ...
        stream.info = {path, method, data, meta}
...
class ZeromqClientStream extends require('stream').Duplex {
  _read (n) {
    log('ZeromqClientStream _read', {n})
  }
  _write (chunk, encoding, callback) {
    log('ZeromqClientStream _write', {chunk, encoding, callback})
    try {
      // WRITE STREAM TO RESOURCE
      this.zeromqClient.send(JSON.stringify({ reqId: this.reqId, type: 'stream', data: chunk }))
      callback()
    } catch (err) {
  ...
  _destroy () {
    ...
    log('ZeromqClientStream _destroy', {})
  }
}
